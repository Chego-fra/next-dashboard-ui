...
const ParentListPage = () => {
  ...
  useEffect(() => {
    const fetchParents = async () => {
      try {
        const response = await fetch("http://localhost:4000/api/getAllParents");
        const data = await response.json();

        // Map the parent data to include associated students
        const formattedData = data.map((parent: any) => ({
          id: parent.id,
          parentName: parent.parentName,
          email: parent.email,
          students: parent.Students?.map((student: any) => student.studentName) || [],
          phone: parent.phone,
